...
contract Liquidations is Owned, MixinResolver, MixinSystemSettings, ILiquidations {
    ...
    function flagAccountForLiquidation(address account) external rateNotInvalid("MNA") {
    ...
    // Check SNX rate is not stale
    function checkAndRemoveAccountInLiquidation(address account) external rateNotInvalid("MNA") {
        systemStatus().requireSystemActive();
