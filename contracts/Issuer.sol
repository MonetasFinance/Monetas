...
//    }
    ...
    function _availableCurrencyKeysWithOptionalSNX(bool withSNX) internal view returns (bytes32[] memory) {
        ...
        if (withSNX) {
            currencyKeys[availableSynths.length] = "MNA";
        }
...
//            }
        ...
        if (currencyKey == "MNA") {
            // if no rate while iterating through synths, then try MNA
            currencyRate = rates[synthsAndSNX.length - 1];
        } else if (currencyRate == 0) {
            // and, in an edge case where the requested rate isn't a synth or MNA, then do the lookup
        ...
        )
    {
        ...
        // if they have 0 MNA, but it's a necessary trade-off
    ...
    function _maxIssuableSynths(address _issuer) internal view returns (uint) {
        // What is the value of their MNA balance in sUSD
        uint destinationValue = exchangeRates().effectiveValue("MNA", _collateral(_issuer), sUSD);
    ...
    function _collateralisationRatio(address _issuer) internal view returns (uint, bool) {
        ...
        (uint debtBalance, , bool anyRateIsInvalid) = _debtBalanceOfAndTotalDebt(_issuer, "MNA");

        // it's more gas intensive to put this check here if they have 0 MNA, but it complies with the interface
    ...
    {
        // How many MNA do they have, excluding escrow?
        // Note: We're excluding escrow here because we're interested in their transferable amount
        // and escrowed MNA are not transferable.

        // How many of those will be locked by the amount they've issued?
        // Assuming issuance ratio is 20%, then issuing 20 MNA of value would require
        // 100 MNA to be locked in their wallet to maintain their collateralisation ratio
        // The locked synthetix value can exceed their balance.
        uint debtBalance;
        (debtBalance, , anyRateIsInvalid) = _debtBalanceOfAndTotalDebt(account, "MNA");
        uint lockedSynthetixValue = debtBalance.divideDecimalRound(getIssuanceRatio());

        // If we exceed the balance, no MNA are transferable, otherwise the difference is.
    ...
    function issueSynthsOnBehalf(
        ...
        require(!anyRateIsInvalid, "A synth or MNA rate is invalid");
    ...
    function _internalIssueSynths(
        ...
        // Store their locked MNA amount to determine their fee % for the period
    ...
    function liquidateDelinquentAccount(
        ...
        // What is the value of their MNA balance in sUSD?
        uint collateralValue = exchangeRates().effectiveValue("MNA", collateralForAccount, sUSD);
        ...
        uint snxRedeemed = exchangeRates().effectiveValue(sUSD, amountToLiquidate, "MNA");

        // Add penalty
        totalRedeemed = snxRedeemed.multiplyDecimal(SafeDecimalMath.unit().add(liquidationPenalty));

        // if total MNA to redeem is greater than account's collateral
